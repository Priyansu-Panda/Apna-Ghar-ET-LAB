...
You can update any text field (title, description, location, price, amenities, availableRooms, status). Optionally, attach a single new image under the `images` key to add it to the property's image gallery.
...
- The `images` field from the request body is always deleted. Images can only be added by uploading a real file.

**What Happens if a file is uploaded:**
1. The new image is uploaded to ImageKit.
2. An image object `{ title, uri, ownerId }` is created.
3. MongoDB's `$push` operator is used to safely *append* the new image to the existing array (not overwrite it).
